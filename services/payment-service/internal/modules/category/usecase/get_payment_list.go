...
func (uc *categoryUsecaseImpl) GetPaymentList(ctx context.Context, filter *domain.FilterCategory) (result domain.ResponseCategoryList, err error) {
	trace, ctx := tracer.StartTraceWithContext(ctx, "CategoryUsecase:GetAllCategory")
	defer trace.Finish()

	data, err := uc.repoSQL.CategoryRepo().FetchPaymentList(ctx, filter)
	if err != nil {
		return result, err
	}
