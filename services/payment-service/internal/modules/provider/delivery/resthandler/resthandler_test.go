...
import (
	...
	mockusecase "payment-service/pkg/mocks/modules/provider/usecase"
...
func TestRestHandler_getAllProvider(t *testing.T) {
	tests := []testCase{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			providerUsecase := &mockusecase.ProviderUsecase{}
			providerUsecase.On("GetAllProvider", mock.Anything, mock.Anything).Return(
				domain.ResponseProviderList{}, tt.wantUsecaseError)
			mockValidator := &mockinterfaces.Validator{}
			mockValidator.On("ValidateDocument", mock.Anything, mock.Anything).Return(tt.wantValidateError)

			uc := &mocksharedusecase.Usecase{}
			uc.On("Provider").Return(providerUsecase)
			...
			handler.getAllProvider(res, req)
			assert.Equal(t, tt.wantRespCode, res.Code)
		})
	}
}

func TestRestHandler_getDetailProviderByID(t *testing.T) {
	tests := []testCase{
		{
			name: "Testcase #1: Positive", wantUsecaseError: nil, wantRespCode: http.StatusOK,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			providerUsecase := &mockusecase.ProviderUsecase{}
			providerUsecase.On("GetDetailProvider", mock.Anything, mock.Anything).Return(domain.ResponseProvider{}, tt.wantUsecaseError)
			mockValidator := &mockinterfaces.Validator{}
			mockValidator.On("ValidateDocument", mock.Anything, mock.Anything).Return(tt.wantValidateError)
			...
			req.Header.Add(candihelper.HeaderContentType, candihelper.HeaderMIMEApplicationJSON)
			res := httptest.NewRecorder()
			handler.getDetailProviderByID(res, req)
			assert.Equal(t, tt.wantRespCode, res.Code)
		})
	}
}

func TestRestHandler_createProvider(t *testing.T) {
	tests := []testCase{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			providerUsecase := &mockusecase.ProviderUsecase{}
			providerUsecase.On("CreateProvider", mock.Anything, mock.Anything).Return(domain.ResponseProvider{}, tt.wantUsecaseError)
			mockValidator := &mockinterfaces.Validator{}
			mockValidator.On("ValidateDocument", mock.Anything, mock.Anything).Return(tt.wantValidateError)
			...
			req.Header.Add(candihelper.HeaderContentType, candihelper.HeaderMIMEApplicationJSON)
			res := httptest.NewRecorder()
			handler.createProvider(res, req)
			assert.Equal(t, tt.wantRespCode, res.Code)
		})
	}
}

func TestRestHandler_updateProvider(t *testing.T) {
	tests := []testCase{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			providerUsecase := &mockusecase.ProviderUsecase{}
			providerUsecase.On("UpdateProvider", mock.Anything, mock.Anything, mock.Anything).Return(tt.wantUsecaseError)
			mockValidator := &mockinterfaces.Validator{}
			mockValidator.On("ValidateDocument", mock.Anything, mock.Anything).Return(tt.wantValidateError)
			...
			req.Header.Add(candihelper.HeaderContentType, candihelper.HeaderMIMEApplicationJSON)
			res := httptest.NewRecorder()
			handler.updateProvider(res, req)
			assert.Equal(t, tt.wantRespCode, res.Code)
		})
	}
}

func TestRestHandler_deleteProvider(t *testing.T) {
	tests := []testCase{
		{
			name: "Testcase #1: Positive", wantUsecaseError: nil, wantRespCode: http.StatusOK,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			providerUsecase := &mockusecase.ProviderUsecase{}
			providerUsecase.On("DeleteProvider", mock.Anything, mock.Anything).Return(tt.wantUsecaseError)
			mockValidator := &mockinterfaces.Validator{}
			mockValidator.On("ValidateDocument", mock.Anything, mock.Anything).Return(tt.wantValidateError)
			...
			req.Header.Add(candihelper.HeaderContentType, candihelper.HeaderMIMEApplicationJSON)
			res := httptest.NewRecorder()
			handler.deleteProvider(res, req)
			assert.Equal(t, tt.wantRespCode, res.Code)
		})
	}
}
